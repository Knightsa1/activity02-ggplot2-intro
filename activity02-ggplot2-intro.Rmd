...
species- a factor denoting penguin denoting penguin species
-island-  island
-bill_length_mm-  bill length (millimeters)
-bill_depth_mm- bill depth (millimeters)
-flipper_length_mm- flipper_leghte(millimeters)
...
**Response**: Box-plot gives an overview of the data, but the jitter gives a look at the spread of data.
...
ggplot(data = penguins, mapping = aes(x = bill_length_mm, y = sex)) +
  geom_boxplot()+  geom_jitter(mapping = aes(color = sex))
```


```{r bill_length_by_island}
ggplot(data = penguins, mapping = aes(x = bill_length_mm, y = island)) +
  geom_boxplot()+  geom_jitter(mapping = aes(color = island))
```

**Response**: I think it would be species because the Adelie and Chinstrap penguins seem to have a higher bill length than Gentoo.
...
**Response**: There is a positive association. There also seems to be cluster of vertical lines at various flipper lengths.
...
**Response**: The Adelie and Chinstrap penguin overlap almost completely, but the Gentoo have a higher body mass and flipper length.
...
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = body_mass_g)) + 
    geom_point(mapping = aes(color = species, shape = species, size = 0.9, alpha = 0.9))
```

Discuss the decisions you made to help your plot be easier to read.

**Response**: Size makes each point easier to see and putting apha on it makes it easier to see the points below.
